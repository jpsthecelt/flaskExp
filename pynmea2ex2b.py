...
def parseGPS(raw_mesg, discardIt):
    ...
    try:
        ...
        return ( timestamp = gmtime(), lat =       0.0, lat_dir =   '0', lon =       0.0, lon_dir =   '0', altitude =  0.0, alt_units = 'M',
             got_fix =   False, num_sats =  0, error_msg = '')
...
def parseGPS(raw_mesg, discardIt):
    ...
    if msg.sentence_type != 'GGA':
        ...
        return (nmea_msg(timestamp=msg.timestamp, lat=(msg.lat or 0.0), lat_dir=msg.lat_dir, lon=(msg.lon or 0.0), lon_dir=msg.lon_dir, 
                altitude=(msg.altitude or 0.0), altitude_units=(msg.altitude_units or 'M')))
...
def update_gps():
    try:
        ...
        parser.add_argument('device',  default='/dev/serial0', help='device from which we will connect for GPS messages')
